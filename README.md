...
每个知识节点就是一章。AI 按固定结构撰写正文：**技术要点 → 深入讲解 → 代码实例 → 常见误区与考点**（人文学科自动换成「实例与证据」），拒绝笼统介绍。写完即持久化，下次打开直接读。

既然看和学是这个应用的主要动作，正文就按**权威 + 结构完整**的契约生成：第一个要点必须先给出可判定的定义，能量化的一律给具体量，并区分**公认结论 / 主流做法 / 仍有分歧**三档确定性；拿不准的宁可标出分歧，也不编造函数名、版本号或论文。撰写时会带上全书目录与同级章节标题，避免章节之间互相重复。

节点详情是**居中的阅读窗口**，正文列限宽 780px 居中，行长按连续阅读调过；正文里支持表格、代码高亮与「复制代码」：

![AI 撰写的章节正文](docs/screenshots/book-reader.png)

### ✂️ 圈选提问 · Select & Ask

书上任何看不懂的地方，圈选即问。浮动的段落级工具条提供四个动作：**圈选提问 / 下钻 / 平铺同类 / 跨域门户**。答案流式返回，并沉淀进该章节的 Q&A 区 —— **你的困惑成为书的一部分**。
...
首次打开即可阅读内置的演示图谱 **「AI Agent 实现原理」**（10 个主干分支 / 81 个知识点，正文全部预置），无需先配置模型也能看到完整效果。
...
│   │   ├── learning-prompts.js        # 知识生成契约（骨架 / 正文 / 下钻 / 平铺 / 门户 / 出题 / 判分 / 提问）
...
│   └── src/
│       ├── pages/LearningMapPage.tsx          # 图谱 / 看板 / 居中详情窗口
